...
##PBI_PARAM: Find granularity parameter automatically  
#Type: logical, Default:TRUE, Range:NA, PossibleValues:NA, Remarks:(1) used to compute eps in dbscan, (2) Time consuming
...
}else
{
  ...
  #find eps
  if(is.null(eps) && autoGranularity)
  ...
  drawColors <- c(outLierCol, rainbow(numOfClusters))
  ...
  plot(dataset[, 1], dataset[, 2], col = alpha(colpoints, pointTransparency), pch = pchPoints, 
       xlab = names(dataset)[1], ylab = names(dataset)[2], xlim=xlim )
