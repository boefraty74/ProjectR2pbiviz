...
names(Time)[1] = "myTime"
ValueAdd = data.frame( v = seq(1,length(Time))/10 + rep(c(1,2,3,4), length.out = length(Time)) + runif(length(Time), min=-1, max=1))
ValueMul = data.frame( v = seq(1,length(Time))/10 * rep(c(1,2,3,4), length.out = length(Time)) * runif(length(Time), min=0.75, max=1.25))
...
minSamples2run = 7 # minimum samples for analysis
  ...
  if(plotType == 'all')
    ...
    if(plotType %in% c('clean', 'byseasonClean'))
      ...
      if(plotType %in% c('trend', 'seasonal', 'reminder'))
        ...
        pbiInfo = paste("Model: ", modelType, ", freq = ", as.character(freqv), ", ", pbiInfo, sep ="")
        print(pbiInfo)
        return(pbiInfo)
}
...
makeLog = function (val)
{
  ...
  return(list(add = add, logVal = logVal, base = exp(1)))
}

makeUnLog = function (logVal, add, base = exp(1))
{
  transVal = exp(logVal)
  val = transVal - add
...
flexTSdecomposition = function(Time, vals, freq, trendSmoothness, myts, robustToOutliers, degree)
{
  ...
  if(freq ==1)
  {
    ...
    #fit <- loess(vals ~ Time, degree = 1+ degree, span = span)
...
# Type: fill, default:'gray', 
# Min: , Max:
infoCol = 'gray'  #default
 ...
 
{
  ...
  if(plotType != "empty")
  {
    ...
    {#compute the same for additive model, compare and select one 
      ...
      evarsAdd = apply(dfTSDadd[,-1], 2, FUN = explained, sig = vals)
  ...
  {
    if(is.null(mvals))
      mts0 = cbind(data=vals, seasonal = dfTSD$seasonal  , trend=dfTSD$trend, remainder=dfTSD$reminder)
    else
      mts0 = cbind(data=avals, seasonal =  makeUnLog(dfTSD$seasonal, mvals$add)  , 
                   trend=makeUnLog(dfTSD$trend,mvals$add), remainder=makeUnLog(dfTSD$reminder,mvals$add))
    ...
    if(!(plotType %in% c("byseason","byseasonClean", "empty")))
    {
      ...
      if(!is.null(mvals))
        dfTSD[,plotType] = makeUnLog(dfTSD[,plotType],mvals$add)
      ...
      abline(a = mean(avals), b = 0, col = "green", lty = 2, col.lab = labelsCol)
      ...
      if(plotType %in% c("byseason","byseasonClean"))
      {
        ...
        monthplot(d , main="", ylab = ylab, xlab = "Season",  
...
#                         plotType = "all", # all, clean, trend, seasonal, reminder, byseason
#                         lineWidth = 40, #
#                         lineCol = "red",
#                         labelsCol = "orange",
#                         labelsFont = 10,
