  ...
  for(g in (1:NG))
  {
    ...
    if(ind == 5)
    {
      ...
      for (p in rcvRdata$groups$listParamInds[[g]])
      {
        ...
        minp = maxp = NULL
        if(length(rcvRdata$params$pmin)>=p)
        {
...
GenerateParamsBlockForRscript = function(rcvRdata)
{
  ...
  lineCount =1
  scriptBlock[lineCount] = "#********* PBI Parameters Block ***************"; lineCount = lineCount + 1
  ...
  for (r  in (1:NR))
  {
    rn = rcvRdata$roles[[r]]$name
    scriptBlock[lineCount] = paste('if(!exists("', rn, '"))', sep ="" );  lineCount =  lineCount + 1
    scriptBlock[lineCount] = paste('\t',rn, " = NULL \n", sep ="" );  lineCount =  lineCount + 1
  }
  ...
  for (g  in (1:NG))
  {
    ...
    for (p  in (1:NP))
    {
      ...
      if(namep =="show" && rnamep !="show" && typep =="bool" )
      {
  ...
  {
    gn = rcvRdata$groups$name[[g]]
    scriptBlock[lineCount] = paste('if(exists("', gn, '_show") && ', gn, "_show == FALSE)",  "", sep ="" );  lineCount =  lineCount + 1
  ...
  for (g  in (1:NG))
  {
    ...
    for (p  in (1:NP))
    {
      ...
      if(namep !="show")
      {
        ...
        typep = rcvRdata$params$type[[ip]]
        minp = maxp = descrp = NULL
        if(length(rcvRdata$params$pmin)>=ip)
        {
          minp = rcvRdata$params$pmin[[ip]]
          maxp = rcvRdata$params$pmax[[ip]]
        }
        defaultp = rcvRdata$params$default[[ip]]
        if(length(rcvRdata$params$description)>=ip)
...
FindFileNameInSourceCall = function(allLines,targetLine = 1, targetSymbol = 1)
{
  #find somthing in between "***" , or '***' and return 
  #TODO: make it more robust 
  outline = NULL
  myline = allLines[targetLine]
  myline = substr(myline, targetSymbol, 1000)
  
  r = as.numeric(gregexpr("'",myline)[[1]])
  if(r[1]==-1)
    r = as.numeric(gregexpr('"',myline)[[1]])
  
  print(r)
  if(r[1]!=-1 && length(r)>1)
    outline = substr(myline, r[1]+1, r[2]-1)
...
pushNewLines = function(outLines,mapNewOutLines) 
{
  # mapNewOutLines = list(lineID= NULL, lineBlocks = list())
  if(!is.null(mapNewOutLines$lineID))
  {
    NI = length(mapNewOutLines$lineID)
    for (ni in seq(NI,1))
    {
      NL = length(outLines)
      t = mapNewOutLines$lineID[ni]
      b = mapNewOutLines$lineBlocks[[ni]]
      outLines = c(outLines[1:(t-1)],b,outLines[(t+1):NL])
    }
  }
  return(outLines)
}
